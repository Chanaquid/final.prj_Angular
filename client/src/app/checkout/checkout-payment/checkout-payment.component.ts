...
export class CheckoutPaymentComponent implements OnInit{
  ...
  ngOnInit(): void {
    loadStripe('pk_test_51NibAkJHVGvlsBTniILorH8O9ZGlqIXihH8Iow8kygjFUnsiG0bU0MRwru5wOfE43NCIhmAFgHFKad2JfYvrTzGA00ll3QgTTu').then(stripe => {
      this.stripe = stripe;
  ...
  async submitOrder()
  {
    ...
    try {
      ...
      const paymementResult = await this.confirmPaymentWithStripe(basket);
      if(paymementResult.paymentIntent) {
        this.basketService.deleteBasket(basket);
        const navigationExtras : NavigationExtras = {state: createdOrder};
        this.router.navigate(['checkout/success'], navigationExtras);
      } else {
        this.toastr.error(paymementResult.error.message);
      }
    } catch (error : any) {
      console.log(error);
